#!/usr/bin/env python3
"""Simple PyTorch multiclass classification example for tabular numerical data.

What this shows:
- NeuralNetwork class with __init__ and forward
- Trainer class with train/evaluate/predict
- Synthetic dataset + DataLoaders
...
class Trainer:
    ...
    def train(self, train_loader, epochs, test_loader=None, out_dir="outputs"):
        ...
        best_test = float("inf")
        best_path = os.path.join(out_dir, "multiclass_best.pt")
...
def make_synthetic(n_samples=2000, input_dim=5, n_classes=3, seed=42):
    rng = np.random.default_rng(seed)
    X = rng.normal(0, 1, size=(n_samples, input_dim)).astype(np.float32)

    # Create simple class centroids and assign by nearest centroid
    centroids = rng.normal(0, 1, size=(n_classes, input_dim)).astype(np.float32)
...
def main():
    ...
    X, y = make_synthetic(n_samples=3000, input_dim=input_dim, n_classes=output_dim)
    ...
    pred_single, prob_single = trainer.predict(sample)
    print("Single prediction (class):", pred_single.cpu().numpy())
    print("Single prediction (probs):", prob_single.cpu().numpy())

    # Batch prediction
    batch = torch.from_numpy(X_test[:10])
    pred_batch, prob_batch = trainer.predict(batch)
    print("Batch prediction shape (preds):", pred_batch.shape)
    print("Batch prediction shape (probs):", prob_batch.shape)
