#!/usr/bin/env python3
"""Transformer encoder-decoder fine-tuning (toy translation).

Downstream task: token-wise shift by +2 (mod vocab).
Loads pretrained model from outputs/transformer_pretrain.pt
Saves finetuned model to outputs/transformer_finetune.pt
"""
...
def main():
    ...
    pre_path = os.path.join(OUT_DIR, "transformer_pretrain.pt")
    ...
    best = float("inf")
    best_path = os.path.join(OUT_DIR, "transformer_finetune.pt")
