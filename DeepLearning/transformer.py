#!/usr/bin/env python3
"""Minimal Transformer (encoder-decoder) in a single file.
...
def main():
    ...
    best_path = os.path.join(OUT_DIR, "transformer_best.pt")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for xb, yb in dl:
            ...
            # teacher forcing: predict full target sequence
            tgt_in = yb  # using full target as input
            tgt_mask = subsequent_mask(tgt_in.size(1)).to(DEVICE)

            opt.zero_grad(set_to_none=True)
            logits = model(xb, tgt_in, tgt_mask=tgt_mask)
    ...
    # Real-time style inference: reload best model and run one input
    infer_model = Transformer(
        VOCAB, EMBED, HEADS, FFN_DIM, LAYERS, DROPOUT, max_len=SEQ_LEN
    ).to(DEVICE)
    ...
    with torch.no_grad():
        ...
        for _ in range(SEQ_LEN):
            ...
            logits = infer_model(src, tgt, tgt_mask=tgt_mask)
