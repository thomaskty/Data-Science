...
\newpage
	...
	To estimate the unknown coefficients, we minimize the \textbf{Residual Sum of Squares (RSS)}:
	...
	We obtain the least squares estimates by differentiating \(RSS\) with respect to
	\(\beta_0\) and \(\beta_1\), and setting the derivatives equal to zero.
	
	\paragraph{Step 1: Differentiate with respect to \(\beta_0\)}
	...
	\paragraph{Step 2: Differentiate with respect to \(\beta_1\)}
	...
	\begin{itemize}
		...
		\item The numerator term appears frequently in statistics and regression, so it is commonly written as:
		...
		\item Similarly, the variance-related quantities are written as:
		...
		\[
		S_{xx} \text{ measures variation in } X
		\]
	
		\[
		S_{yy} \text{ measures variation in } Y
		\]
	
		\[
		S_{xy} \text{ measures joint variation between } X \text{ and } Y
		\]
	
		\item The \textbf{magnitude of covariance} depends on the scale of the variables, making it difficult to compare relationships across datasets.
	
		\item \textbf{Correlation} is the normalized form of covariance.
		It removes scale dependence by dividing covariance by the product of standard deviations.
		...
		\item Since correlation is normalized, it always lies in the range:
		...
		\item Covariance and correlation describe association, not causation.
		A strong correlation does not imply that one variable causes the other.
	
		\item The \(S_{xx}, S_{yy}, S_{xy}\) notation is extremely important in linear regression because the slope estimator can be written compactly as:
	...
	\paragraph{Step 1: Expand the Quadratic Form}
	...
	\paragraph{Step 2: Use Scalar Transpose Property}
	A very important identity is: $s^\top = s$ for any scalar \(s\).
	...
	\paragraph{Step 3: Differentiate with Respect to \(w\)}

	\textbf{Identity 1:}
	
	\[
	\frac{\partial}{\partial w}(a^\top w)
	=
	a
	\]
	...
	\[
	\frac{\partial}{\partial w}(w^\top A w)
	=
	(A + A^\top)w
	\]
	...
	\paragraph{Step 4: Set Gradient Equal to Zero}
	...
	\subsubsection{Derivation of Gradient -- Matrix Form}
	...
	\paragraph{Gradient with Respect to Weight \(w_k\)}
	
	We compute:
	
	\[
	\frac{\partial RSS}{\partial w_k}
	=
	...
	\sum_{i=1}^{n}
	2(y_i - \hat{y}_i)
	\frac{\partial}{\partial w_k}(y_i - \hat{y}_i)
	\]
	...
	\hat{y}_i
	=
	w_1x_{i1} + w_2x_{i2} + \cdots + w_kx_{ik} + \cdots + b
	\]
	...
	\paragraph{Gradient with Respect to Bias \(b\)}
	...
	\sum_{i=1}^{n}
	2(y_i - \hat{y}_i)
	\frac{\partial}{\partial b}(y_i - \hat{y}_i)
	\]
	...
	\paragraph{Important Intuition}
	
	The feature values appear in the weight gradient because:
	...
	\subsubsection{Update Rule \& Formula}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Objective : $\min_\beta \|y - X\beta\|^2 + \lambda \|\beta\|^2$
			 \item Closed Form Solution : $\hat{\beta} = (X^T X + \lambda I)^{-1} X^T y $
			 \item Gradient Descent Solution : $\nabla = -2X^T (y - X\beta) + 2\lambda \beta$
		...
		\begin{itemize}
			...
			\item Objective : $\min_\beta \|y - X\beta\|^2 + \lambda \|\beta\|_1$
			\item Closed Form Solution : No closed-form solution
			\item Gradient (Subgradient) : $\nabla = -2X^T (y - X\beta) + \lambda \, \text{sign}(\beta)$
		\end{itemize}
		...
		\begin{itemize}
			...
			\item Objective : $\min_\beta \|y - X\beta\|^2 + \lambda_1 \|\beta\|_1 + \lambda_2 \|\beta\|^2$
			\item Closed Form Solution : No closed-form solution
	...
	\begin{itemize}
	    \item Leverage measures how unusual a point’s predictor values are.
	    
	    \item A point far away in feature space has high leverage.
	    ...
	    \item The diagonal elements of the hat matrix, $h_{ii}$, are called leverage values.
	    
	    \item High leverage points correspond to observations with unusual predictor values.
	...
	\begin{itemize}
	    \item Cook's Distance measures how much the fitted model changes if an observation is removed.
	    
	    \item It combines residual magnitude and leverage:
	    $$
	    D_i =
	    ...
	    \item $e_i$ = residual for observation $i$
	    
	    \item $h_{ii}$ = leverage of observation $i$
	    
	    \item $p$ = number of parameters (including intercept)
	    
	    \item $MSE = \dfrac{RSS}{n-p}$
	    
	    \item $D_i > 1$ may indicate a highly influential point.
	\end{itemize}
	...
	\subsection{Likelihood, AIC, and BIC in Linear Regression}
	...
	\paragraph{Maximum Likelihood Estimate of Variance}
	...
	\begin{itemize}
		...
		\item \(\log L\) = maximized log-likelihood
	\end{itemize}
	...
	Both AIC and BIC balance:
	
	\begin{enumerate}
		\item Goodness of fit
		\[
		\text{Lower RSS} \Rightarrow \text{Higher likelihood}
		\]
	
		\item Model complexity
		\[
		\text{More parameters} \Rightarrow \text{Higher penalty}
		\]
	\end{enumerate}
	...
	\begin{itemize}
		\item Adding predictors usually decreases RSS
		\item But adding too many predictors increases model complexity
		\item AIC and BIC penalize unnecessarily complex models
	\end{itemize}
	
	\vspace{0.5em}
	
	\paragraph{Difference Between AIC and BIC}
	...
	\subsection{Standard Errors of Logistic Regression Coefficients}
	
	\begin{itemize}
	    \item Coefficients are estimated using \textbf{Maximum Likelihood Estimation (MLE)}.
	    
	    \item Hessian / Fisher Information Matrix:
	    $$
	    H = X^T W X
	    $$
	    ...
	    \item Covariance matrix of coefficients:
	    $$
	    \mathrm{Cov}(\hat{\beta}) = (X^T W X)^{-1}
	    $$
	    
	    \item Standard error of coefficient $\hat{\beta}_j$ is given by:
	    $$
	    SE(\hat{\beta}_j)=\sqrt{\left[(X^T W X)^{-1}\right]_{jj}}
	    $$
	    
	    \item Standard errors quantify uncertainty in coefficient estimates.
	    
	    \item Used in Wald Test, confidence intervals, and p-value computation.
	\end{itemize}
	
	\subsection{Hypothesis Testing \& Likelihood Ratio Test}
	
	\begin{itemize}
	    \item \textbf{Wald Test}: Most common coefficient significance test.
	    $$
	    ...
	    \item \textbf{Likelihood Ratio Test} for overall model significance:
	    $$
	    H_0:\beta_1 = \beta_2 = \cdots = \beta_p = 0
	    $$
	    
	    \item In this test, we compare the null model (intercept only) with the full model.
	    
	    \item Likelihood Ratio Statistic is given by:
	    $$
	    LR=-2\left[\log L_0 - \log L_1\right]
	    $$
	...
	\begin{itemize}
	    \item Leverage measures how unusual an observation's predictor values are.
	    
	    \item Leverage matrix:
	    $$
	    H = W^{1/2}X(X^T W X)^{-1}X^T W^{1/2}
	    $$
	    
	    \item Leverage values are diagonal elements:
	    $$
	    h_i = H_{ii}
	    $$
	    
	    \item High leverage points can strongly influence model fitting.
	\end{itemize}
	...
	\begin{itemize}
	    ...
	    \item $r_i$ = standardized residual
	    
	    \item $h_i$ = leverage
	    
	    \item $p$ = number of parameters
	    
	    \item Large Cook's Distance indicates influential observations.
	    
	    \item Common threshold:
	    $$
	    D_i > \frac{4}{n}
	    $$
	...
	\subsection{Goodness of Fit in Logistic Regression}
	
	\begin{itemize}
	    \item Goodness of fit measures how well the model explains observed outcomes.
	    
	    \item Residual Deviance is the primary likelihood-based fit metric:
	    $$
	    D = -2(\log L_{\text{model}} - \log L_{\text{saturated}})
	    $$
	    
	    \item Lower deviance indicates better model fit.
	    
	    \item \textbf{Hosmer--Lemeshow Test} compares observed vs predicted probabilities across groups.
	    ...
	    \item Hosmer--Lemeshow test statistic:
	    $$
	    HL =
	    \sum_{g=1}^{G}
	    \left(
	    ...
	    \item Test statistic approximately follows:
	    $$
	    HL \sim \chi^2_{g-2}
	    $$
	    where $g$ is the number of groups.
	    
	    \item Large p-value $(>0.05)$ indicates good fit, whereas small p-value $(<0.05)$ indicates poor fit.
	\end{itemize}
	
	
	\subsection{McFadden $R^2$ in Logistic Regression}
	
	\begin{itemize}
	    \item Logistic regression has no true variance-based $R^2$.
	    
	    \item McFadden $R^2$ is a likelihood-based pseudo $R^2$ measure.
	    ...
	    \item $\log L_{\text{full}}$ = log-likelihood of fitted model
	    
	    \item $\log L_{\text{null}}$ = log-likelihood of intercept-only model
	    
	    \item Larger values indicate better model fit.
	    
	    \item Typical good values lie between $0.2 - 0.4$.
	    
	    \item Uses the same likelihood principle as the Likelihood Ratio Test.
	\end{itemize}
	
	
	\subsection{Box--Tidwell Test (Linearity of Log-Odds)}
	
	\begin{itemize}
	    \item Used to check whether predictors have a linear relationship with log-odds in logistic regression.
	    
	    \item For predictor $x$, create interaction term:
	    $$
	    x \log(x)
	    $$
	    
	    \item Extended logistic regression model:
	    $$
	    ...
	    \item Applied separately for each continuous predictor.
	\end{itemize}
	...
	\end{itemize}
		...
		\begin{itemize}
		    \item $w$ is the weight vector (normal vector),
		...
		A hyperplane in $\mathbb{R}^{d}$ is a $(d-1)$ dimensional affine subspace.
		
		Examples:
		
		\begin{itemize}
		    \item 2D $\rightarrow$ line
		    \item 3D $\rightarrow$ plane
		    \item higher dimensions $\rightarrow$ hyperplane
		\end{itemize}
		...
		represents a signed scaled distance from the hyperplane.
		
		The true perpendicular distance is
		...
		directly becomes the signed perpendicular distance.
		
		\subsection{Projection Interpretation}
		
		If $w$ is a unit normal vector, then
		
		\[
		w^\top x
		\]
		
		is the projection of $x$ onto the direction $w$.
		
		Take a point $x_0$ on the hyperplane:
		...
		Thus, $-b$ acts as the cutoff threshold along the direction $w$.
		...
		implies the point lies on one side of the hyperplane, while
		
		\[
		w^\top x < -b
		\]
		
		implies it lies on the other side.
		
		Thus, a linear classifier can be interpreted as:
		
		\begin{enumerate}
		    \item Project onto direction $w$
		    \item Threshold at $-b$
		\end{enumerate}
		...
		Hence the total margin width is
		
		\[
		\frac{2}{\|w\|}
		\]
		
		Maximizing margin is therefore equivalent to minimizing $\|w\|$.
		
		\subsection{Support Vectors}
		
		Support vectors are the points closest to the decision boundary.
		
		For hard-margin SVM, they satisfy
		...
		For soft-margin SVM, support vectors are all points satisfying
		...
		\begin{itemize}
		    ...
		    \item points inside the margin,
		    \item misclassified points.
		\end{itemize}
		
		Only support vectors contribute nonzero hinge loss.
		
		\subsection{Hard Margin SVM}
		
		Hard-margin SVM assumes perfectly linearly separable data.
		...
		\begin{itemize}
		    \item positive-class points have scores $\ge +1$,
		    \item negative-class points have scores $\le -1$.
		\end{itemize}
		
		The values $\pm1$ arise due to scaling normalization.
		...
		represent the same geometric boundary. SVM removes this scale ambiguity by fixing the functional margin to $1$.
		
		\subsection{Soft Margin Primal Form}
		
		Real datasets are usually not perfectly separable.
		...
		\[
		y_i(w^\top x_i+b)\ge1-\xi_i,\ \forall i
		\]
		...
		\begin{itemize}
		    \item $\xi_i=0$ : correctly classified and outside margin
		    \item $0<\xi_i<1$ : correctly classified but inside margin
		...
		\begin{itemize}
		    ...
		    \item minimizing classification violations.
		\end{itemize}
		
		\subsection{Hinge Loss Derivation}
		...
		the optimal slack variable becomes
		...
		This gives the hinge loss:
		
		\[
		L_{\text{hinge}}(x_i,y_i)=
		\max\left(0,\ 1-y_i(w^\top x_i+b)\right)
		\]
		
		Thus, the unconstrained primal objective becomes
		...
		\subsection{Interpretation of Hinge Loss}
		...
		\begin{enumerate}
		    ...
		    Large hinge loss penalty.
		\end{enumerate}
		
		Thus SVM does not merely enforce correct classification; it enforces sufficiently confident classification.
		
		\subsection{Functional vs Geometric Margin}
		...
		represents the true perpendicular distance from the hyperplane.
		
		SVM fixes the functional margin to $1$ so that maximizing geometric margin reduces to minimizing $\|w\|$.
		
		\subsection{Gradients of the Soft-Margin Objective}
		...
		Larger $\lambda$ encourages smaller $\|w\|$, which produces a wider margin:
		
		\[
		\frac{2}{\|w\|}
		\]
		
		Thus regularization directly controls geometric separation and model complexity.
		
		\subsection{Important Insight}
		
		SVM fundamentally seeks a hyperplane that:
		
		\begin{itemize}
		    \item separates the classes,
		    \item maximizes geometric margin,
		    \item allows soft violations through hinge loss,
		    \item focuses optimization only on difficult boundary points.
		\end{itemize}
...
\subsection{Comparison of K-Means \& GMM in terms of EM Algorithm}
	...
	\begin{itemize}
	    \item Train on \(n-1\) observations
	    \item Test on 1 observation
	    \item Repeat \(n\) times
	\end{itemize}
	
	The final error is the average validation error.
	
	\subsubsection{Leave-\(p\)-Out Cross Validation}
	In leave-\(p\)-out cross validation:
	\begin{itemize}
	    \item Leave \(p\) observations for testing
	    \item Train on the remaining \(n-p\) observations
	    \item Repeat for all possible combinations
	\end{itemize}
	...
	\begin{itemize}
	    \item Train on past observations
	    \item Test on future observations
	\end{itemize}
